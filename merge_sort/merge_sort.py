...
def merge(A, aux, low, mid, high):
    ...
    #while there are elements on the left and right runs
...
#sort list A[low..high] using auxilary list aux
def mergesort(A, aux, low, high):
    ...
    #use recursion to split runs into two halves until run size <= 1
    #then merge them and return up call chain
    mergesort(A, aux, low, mid) # split/merge left half
    mergesort(A, aux, mid + 1, high) #split/merge right half

    merge(A, aux, low, mid, high)   # merge the two halfs
